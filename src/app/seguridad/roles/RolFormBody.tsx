...
// Cuerpo del formulario de rol (nombre, descripción y árbol de
// módulos/permisos) sin ningún envoltorio de presentación — lo usa tanto
// RolModal (para "Nuevo Rol") como la edición inline dentro de la fila
// expandida en page.tsx (para "Editar Rol"), así la lógica del árbol de
// permisos vive en un solo lugar.
export default function RolFormBody({ rol, onCancel, onSave }: Props) {
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-slate-800">Módulos y Permisos</h3>
            {totalAsignados > 0 && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-[#eef3ff] text-brand-700">
                {totalAsignados} módulo{totalAsignados !== 1 ? "s" : ""} asignado
                {totalAsignados !== 1 ? "s" : ""}
              </span>
            )}
          </div>
