...
export default function GestionComiteCredito2Page() {
  ...
  const buscar = async (opts: { preservePagina?: boolean } = {}) => {
    try {
      ...
      const data =
        await solicitudesService.getSolicitudesParaComiteCredito2(usuarioId);
  ...
  async function exportarExcel() {
    ...
    const data = solicitudes.map((s) => {
      const fechaEstimada =
        (s as any).sol_fecha_estimada_comite_credito_2 ||
    ...
    XLSX.writeFile(
      wb,
      `solicitudes-comite-credito-2-${new Date().toISOString().slice(0, 10)}.xlsx`,
    );
  }
                    ...
                    {solicitudesActuales.map((solicitud) => {
                      ...
                      return (
                        <Tr key={solicitud.sol_id ?? solicitud.sa_sol_id}>
                          ...
                          <Td className="whitespace-nowrap">
                            {formatDateTime(
                              solicitud.sol_fecha_real_comite_credito_1,
                            )}
                          </Td>
                          <Td className="whitespace-nowrap">
                            {formatDate(fechaEstimada)}
