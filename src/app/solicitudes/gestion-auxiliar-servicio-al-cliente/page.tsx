...
export default function AprobacionDesaprobacionPage() {
  ...
  const buscar = async (opts: { preservePagina?: boolean } = {}) => {
    try {
      ...
      const mapped = data
        ...
        .filter((s: Solicitud) => {
          const cumpleEjecutivo = ejecutivoSeleccionado ? s.sol_ejng_id === ejecutivoSeleccionado : true;
  ...
  async function exportarExcel() {
    ...
    const data = solicitudes.map((s) => {
      const fechaEstimada = (s as any).sol_fecha_est_gest_asc;
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {solicitudesActuales.map((solicitud) => {
                        const fechaEstimada = (solicitud as any).sol_fecha_est_gest_asc;
