  ...
  return (
    <section className="mb-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-wide text-slate-700">Pasos de tu solicitud</p>
          <p className="mt-1 text-xs text-slate-500">Completa cada paso en orden para enviar la solicitud.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3 md:items-stretch">
        {pasos.map((paso, index) => {
          ...
          const puedeAbrir =
            Boolean(onStepClick) &&
            (index === 0 || (index === 1 && estado === "actual") || (index === 2 && estado === "actual"));

          let descripcion = paso.descripcion;
          if (index === 0 && estado !== "completo") {
            descripcion =
              formularioPorcentaje > 0 ? `${formularioPorcentaje}% completado` : "Aún no has iniciado el formulario";
          ...
          return (
            <div key={`${paso.titulo}-wrapper`} className="relative h-full">
              <button
                key={paso.titulo}
                type="button"
                onClick={() => puedeAbrir && onStepClick?.(index)}
                disabled={!puedeAbrir}
                className={`group h-full min-h-40 w-full rounded-xl border p-4 text-left transition-all ${
                  estado === "completo"
                ...
                <div className="mt-4">
                  ...
                  {index === 0 && (
                    <span className="mt-3 inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-600 ring-1 ring-slate-200">
                      {formularioPorcentaje}% del formulario
                    </span>
                  )}
                </div>
              </button>
              {index < pasos.length - 1 && (
                <div
                  aria-hidden="true"
                  className="absolute -right-2 top-1/2 z-10 hidden h-0.5 w-4 -translate-y-1/2 bg-slate-200 md:block"
                />
              )}
            </div>
          );
