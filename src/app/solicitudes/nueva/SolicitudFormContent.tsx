...
export default function SolicitudFormContent({
  ...
  const getValidationRules = (pregunta: FormularioPregunta): ValidationRule => {
    ...
    if (pregunta.fp_tipo === "NUMERO") {
      rules.type = "number";
      if (pregunta.fp_minimo !== undefined) {
        rules.custom = (value) => {
          if (
            value !== undefined &&
            value !== null &&
      ...
      if (pregunta.fp_maximo !== undefined) {
  ...
  const validateCurrentSection = (): boolean => {
    ...
    seccionActual.preguntas.forEach((pregunta) => {
      ...
      if (pregunta.fp_tipo === TIPOS_PREGUNTA.DOCUMENTOS_TABLA) {
        ...
        if (requiereFecha) {
          const fechaEmision = respuestas[pregunta.fp_id]?.valor_fecha;
          if (!fechaEmision) {
  ...
  const shouldShowQuestion = (pregunta: FormularioPregunta): boolean => {
    ...
    const check = (actual: FormularioPregunta): boolean => {
      ...
      // DEBUG: solo para pregunta 1149
      if (actual.fp_id === 1149 && respuestaPadre) {
        console.log(
          "[DEBUG] 1149: respuestaPadre:",
          respuestaPadre,
          "disparador:",
          valorDisparador,
        );
      ...
      if (respuestaPadre.valor_texto) {
        const match = normalize(respuestaPadre.valor_texto) === valorDisparador;
        if (actual.fp_id === 1149)
          console.log("[DEBUG] 1149: match texto:", match);
        return match;
      }
      ...
      if (respuestaPadre.valor_numero !== undefined) {
        ...
        if (actual.fp_id === 1149)
          console.log("[DEBUG] 1149: match numero:", match);
        return match;
      }

      if (respuestaPadre.valor_fecha) {
        const match = normalize(respuestaPadre.valor_fecha) === valorDisparador;
        if (actual.fp_id === 1149)
          console.log("[DEBUG] 1149: match fecha:", match);
        return match;
      }
      ...
      if (respuestaPadre.valor_opcion_id !== undefined) {
        ...
        const descripciones = ids
          .map((id) => {
            const opcion = preguntaPadre.opciones?.find((o) => o.op_id === id);
            return opcion ? opcion.op_descripcion : id;
          })
          .map((value) => normalize(value));

        const match = descripciones.includes(valorDisparador);
        if (actual.fp_id === 1149) {
          console.log("[DEBUG] 1149: ids:", ids);
          console.log("[DEBUG] 1149: descripciones:", descripciones);
          console.log("[DEBUG] 1149: match opcion:", match);
        }
      ...
      if (actual.fp_id === 1149)
        console.log("[DEBUG] 1149: sin valor, retorna false");
      return false;
    };
  ...
  const hasValorEnRespuesta = (
    ...
    if (
      ...
      !Number.isNaN(respuesta.valor_numero)
    ) {
      return true;
    }
  ...
  const shouldShowConditionalField = (
    ...
    const opcionSeleccionada = pregunta.opciones?.find(
      (o) => o.op_id === respuestaActual.valor_opcion_id,
    );
  ...
  const handleGuardar = async () => {
    ...
    for (const doc of documentosConVigencia) {
      ...
      if (tieneVigencia) {
        ...
        const tieneFecha = preguntaFechaHija
