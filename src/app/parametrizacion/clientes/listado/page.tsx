...
export default function ClientesPage() {
  ...
  const [centroSeleccionado, setCentroSeleccionado] = useState<number | undefined>(undefined);
  ...
  const fetchClientesList = async (centro: number | undefined) => {
    try {
      setLoading(true);
      setError(null);
      const clientesData = centro ? await clientesService.getClientesByCentro(centro) : await clientesService.getAll();
  ...
  }, [centroSeleccionado, searchInputValue, searchTerm, filterNit, filterDireccion, hasSearched, currentPage]);
  ...
  const direccionSugerencias = useMemo(() => clientes.map((c) => c.cli_direccion ?? ""), [clientes]);

  // Filtrar clientes
  const filteredClientes = clientes.filter((cliente) => {
    // Filtro por búsqueda (razón social)
    const matchesSearch =
      searchTerm === "" || (cliente.cli_razon_social ?? "").toLowerCase().includes(searchTerm.toLowerCase());

    // Filtro por NIT
    const matchesNit =
      filterNit === "" || (cliente.cli_nro_identificacion ?? "").toLowerCase().includes(filterNit.toLowerCase());

    // Filtro por Dirección
    const matchesDireccion =
      filterDireccion === "" || (cliente.cli_direccion ?? "").toLowerCase().includes(filterDireccion.toLowerCase());
    ...
    <div className="min-h-screen bg-gradient-to-b from-page-from to-page-to p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <FilterField label="Centro de Operación">
              <select
                ...
                onChange={(e) => {
                  setCentroSeleccionado(e.target.value ? Number(e.target.value) : undefined);
                  setCurrentPage(1);
                }}
        ...
        {!hasSearched ? (
          ...
          <EmptyStateCard
            icon={Building}
            title={clientes.length === 0 ? "No hay clientes registrados" : "No se encontraron resultados"}
            subtitle={
              clientes.length === 0 ? "Comienza creando tu primer cliente" : "Intenta con otros términos de búsqueda"
            }
            action={
              clientes.length === 0 ? (
                <button
          ...
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
            <ResultsToolbar count={filteredClientes.length} label="cliente(s)" onExport={handleDownloadExcel} />
            <TableContainer>
                  ...
                  {filteredClientes
                    ...
                    .map((cliente) => (
                      <Tr key={cliente.cli_id}>
                        <Td className="font-semibold text-gray-900">{cliente.cli_razon_social}</Td>
                        <Td className="font-mono">{cliente.cli_nro_identificacion || "-"}</Td>
                        <Td>{cliente.cli_direccion || "-"}</Td>
                        <Td>
                          {cliente.ejecutivo ? (
                            <span className="text-sm font-medium text-gray-900">{cliente.ejecutivo.nombre}</span>
                          ) : (
                        ...
                        <Td sticky align="right">
                          <div className="flex space-x-2 justify-end">
                            <button
                              onClick={() => setViewClienteId(cliente.cli_id)}
                              className="p-2 bg-blue-50 text-brand-600 rounded-lg hover:bg-blue-100 transition"
                              title="Ver detalles">
                              <Eye className="w-4 h-4" />
                            </button>
                            <button
                              onClick={() => goToCliente(`/parametrizacion/clientes/${cliente.cli_id}/editar`)}
                              className="p-2 bg-blue-50 text-brand-600 rounded-lg hover:bg-blue-100 transition"
                              title="Editar">
