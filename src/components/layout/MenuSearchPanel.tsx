...
export function MenuSearchPanel({
  ...
  const starIdleClass = isDark
    ? "text-white/40 hover:text-white"
    : "text-[#cbd5e1] hover:text-amber-500";
    ...
    return (
      <div key={modulo.mod_id} className="group flex items-center gap-1">
        <Link
          href={ruta}
          onClick={onNavigate}
          className={`flex-1 min-w-0 ${rowClass(activo)}`}
        >
        ...
        <button
          ...
          className={`shrink-0 p-1.5 rounded-md transition-colors ${
            favorito ? "text-amber-400" : starIdleClass
          }`}
        >
  ...
  return (
    <div className="space-y-2">
      ...
      {term ? (
        ...
        <div className="space-y-0.5 max-h-72 overflow-y-auto">
          <p
            className={`px-2.5 pt-1 pb-1 text-[10.5px] font-bold uppercase tracking-wide ${sectionLabelClass}`}
          >
            Favoritos
          </p>
          {favoritos.map((f) => (
